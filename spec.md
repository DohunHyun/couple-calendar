...
> v3.0: 메인 기능 **기기 캘린더 연동**(§8-A) 설계 추가. 기존 구글 동기화(§8)는 이로 대체 예정.
...
| **기기 캘린더 연동** | 폰→서비스 단방향 | 🔜 | **메인 기능, 미구현** — 설계 [§8-A](#8-a-기기-캘린더-연동-native-calendar-sync-v30-신규) |
...
- 🔜 **v3.0 추가 예정**: sourceType에 `DEVICE` 추가. 이벤트별 공유 override를 위한 `visibility`(enum PRIVATE/SHARED) 컬럼 추가 — DEVICE 일정은 카테고리 타입이 아니라 이 필드로 공유 결정(기본=설정값, sticky). `externalEventId`=기기 이벤트 ID, `externalCalendarId`=기기 캘린더 ID로 재사용.

### category (`Category`)
id / name(≤30) / colorHex(7) / type(enum PRIVATE,SHARED) / user_id(FK,NN 소유자).
- 🔜 연동한 기기 캘린더별로 카테고리 자동 생성(색 보존). 매핑 키: (owner, externalCalendarId).

### user_settings (`UserSetting`)
googleVisible(기본 true) / ddayVisible(기본 true) / profileCompleted(기본 false).
- 🔜 **v3.0 추가 예정**: `deviceSyncDefaultShared`(기본 false=PRIVATE) — 동기화 일정 기본 공개범위. 연동할 기기 캘린더 ID 목록은 클라이언트 로컬 저장(+선택적 백엔드 동기화).
...
| PUT | `/auth/device-token` | body `{deviceToken}` — 현재 사용자 푸시 토큰 갱신(G3) |
...
| 🔜 POST | `/events/device-sync` | DEVICE 일정 배치 upsert (§8-A). `{ calendars:[{externalCalendarId, name, colorHex}], events:[{externalEventId, externalCalendarId, title, allDay, startAt, endAt, alertOption?}], rangeStart, rangeEnd }` → 범위 내 없어진 DEVICE 일정은 hidden 처리 |
...
| PATCH | `/settings/preferences` | `{googleVisible?, ddayVisible?}` (🔜 `deviceSyncDefaultShared?` 추가) |
...
## 8. Google Calendar 동기화 — `GoogleCalendarSyncService` ⚠️ (🔜 §8-A로 대체 예정)

> **현재 구현(as-built)**. v3.0에서 기기 캘린더 연동(§8-A)이 이를 대체한다. 공휴일은 앱 자체 데이터로 분리 예정.
...
## 8-A. 기기 캘린더 연동 (Native Calendar Sync) — v3.0 신규 🔜

> 메인 기능. 폰 기본 캘린더 → 서비스 단방향. **미구현(설계 확정)**. 제품 정의는 [PRD §3-A](PRD.md).

### 핵심 아키텍처 차이 (vs 구글 동기화)
구글 동기화는 **백엔드**가 토큰으로 직접 fetch했다. 기기 일정은 **단말에서만** 접근 가능하므로 **클라이언트가 읽어 백엔드로 업로드**한다.

```
[폰 캘린더 저장소] --(Capacitor 플러그인 read)--> [앱] --(POST /events/device-sync)--> [백엔드] --> [상대 앱 GET /events]
   EventKit(iOS) /                                                  upsert sourceType=DEVICE
   CalendarContract(Android)
```

### 플러그인
Capacitor 캘린더 플러그인(예: `@ebarooni/capacitor-calendar`)으로 EventKit/CalendarContract 접근. 권한: iOS는 캘린더 읽기 권한, Android는 `READ_CALENDAR`. 웹/프리뷰는 no-op(`Capacitor.isNativePlatform()` 가드, §G3 푸시와 동일 패턴).

### 동기화 흐름 (클라이언트)
1. 권한 요청 → 거부 시 동기화 비활성.
2. 사용자가 **연동 캘린더 선택**(설정). 선택 ID 목록 로컬 저장.
3. 트리거 = 앱 포그라운드 진입 + 당겨서 새로고침. (실시간 백그라운드 ❌ — 모바일 한계)
4. 선택 캘린더에서 조회 범위(현재 월 ± 버퍼)의 일정을 읽어 `POST /events/device-sync`로 배치 업로드.

### 백엔드 upsert (`/events/device-sync`)
- 각 일정 upsert 키 `(owner, sourceType=DEVICE, externalEventId)`.
- 캘린더 메타로 카테고리 보장: `(owner, externalCalendarId)`별 카테고리 자동 생성/갱신, `colorHex` 보존(`ensureCategory` 패턴 재사용).
- **공유(visibility)**: 신규 일정은 `UserSetting.deviceSyncDefaultShared` 기본값 적용. **기존 일정은 사용자가 준 override 유지(sticky)** — 재동기화가 visibility를 덮어쓰지 않음.
- **삭제 전파**: 요청의 `rangeStart~rangeEnd` 안에서, 이번에 안 올라온 기존 DEVICE 일정은 `hidden=true`(기기에서 지워진 것으로 간주).

### 가시성 모델 변경 (중요)
현재는 일정의 PRIVATE/SHARED가 **카테고리 타입**으로 결정된다. DEVICE 일정은 **일정별 override**가 필요하므로 **이벤트 레벨 `visibility` 필드**를 도입한다:
- DEVICE 일정: `event.visibility`가 공유 판정의 source of truth (카테고리 색은 시각용).
- LOCAL/기존 일정: 당분간 카테고리 타입 유지(점진 이행). 목록 조회·`isVisibleToUser`에서 DEVICE는 `visibility` 우선 적용하도록 확장.

### 앱 내 편집 정책 (단방향)
- DEVICE 일정은 앱에서 **공유 토글(visibility)만** 변경 가능. 제목·시간·삭제는 폰 캘린더에서(앱→폰 쓰기는 v1 비범위).
- LOCAL 인앱 일정은 기존대로 전체 CRUD 유지(공존).

### 구글 동기화 정리 (F33)
- `GoogleCalendarSyncService` / `syncGoogle` 경로 제거 또는 비활성.
- 한국 공휴일: 구글 캘린더 의존 제거 → 앱 내장 공휴일 데이터(또는 공휴일 전용 소스)로 대체.
- `googleVisible` 설정 → 일반 "동기화 일정 표시" 토글로 일반화.

## 8.5 알림 / 푸시 (R12)

**수신자 분기** — `PushNotificationService.recipientsFor`: PRIVATE → 작성자 본인, SHARED → 커플 2인. 각 수신자의 `deviceToken`으로 발송, 토큰 없으면 스킵.

**발송 추상화(G3)** — `PushSender` 인터페이스. 기본 구현 `LoggingPushSender`(로그 출력)는 `app.push.provider=log`(기본, `matchIfMissing`)일 때 활성. 실제 FCM/APNS는 동일 인터페이스를 구현한 빈을 추가하고 `app.push.provider`를 전환하면 호출부 변경 없이 동작.

**디바이스 토큰 등록(G3)** — `PUT /auth/device-token`(`{deviceToken}`)으로 현재 사용자의 `User.deviceToken` 갱신. 프론트는 네이티브 앱에서 `@capacitor/push-notifications`로 등록 후 토큰을 이 엔드포인트로 전송(`src/native/push.js`, 웹/프리뷰는 no-op). **남은 것: Firebase(FCM)/APNS 프로비저닝 + 백엔드 `FcmPushSender`** 구현 시 실발송 완성.
...
| ~~G3~~ | 푸시가 로그만 출력 | 🟡 부분 — 추상화 + `PUT /auth/device-token` + Capacitor 푸시 등록 배선. 실제 FCM/APNS는 Firebase·Apple 프로비저닝 + `FcmPushSender` 필요 |
...
| 🔜 NEW | **기기 캘린더 연동 (메인 기능)** | 미구현(설계 확정) — 설계 §8-A, 구현 시 sourceType `DEVICE`·이벤트 `visibility`·`/events/device-sync`·캘린더 플러그인 |
